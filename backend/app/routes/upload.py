...
from app.services.material_service import process_material, get_user_materials, delete_material
...
async def upload(
    ...
    logger.info(f"Stage: File upload started - {file.filename} by user {current_user.id}")
    ...
    if auto_create_notebook == "true" and not nb_id:
        ...
        # Create the notebook
        created_notebook = await create_notebook(current_user.id, notebook_name, None, db)
