...
async def generate_podcast(
    ...
    audio_buffer, title, dialogue_timing = generate_podcast_audio(
    ...
    filename = (
        f"{safe_title.replace(' ', '_')[:50] or 'podcast'}_{uuid.uuid4().hex[:6]}.wav"
    )
...
async def get_podcast_audio(user_id: str, filename: str):
    ...
    return FileResponse(
        path=audio_path, media_type="audio/wav", headers={"Cache-Control": "no-cache"}
    )
...
async def download_podcast(
    ...
    audio_buffer, title, _ = generate_podcast_audio(material.original_text)
