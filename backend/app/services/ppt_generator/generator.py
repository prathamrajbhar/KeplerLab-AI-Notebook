...
class PPTGenerator:
    ...
    def _add_title_slide(self, prs: Presentation, title: str):
        ...
        la = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(0.5), Inches(2.2), Inches(0.08), Inches(2.5)
        )
        la.fill.solid()
        ...
        # Decorative circles
        for x, y, s, c in [
            (10.5, 0.6, 0.5, Colors.ACCENT_PRIMARY),
            (11.2, 0.9, 0.35, Colors.ACCENT_SECONDARY),
            (11.7, 0.5, 0.25, Colors.ACCENT_CYAN),
        ]:
            circ = slide.shapes.add_shape(
                MSO_SHAPE.OVAL, Inches(x), Inches(y), Inches(s), Inches(s)
            )
            circ.fill.solid()
        ...
        ul = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(0.8), Inches(5.6), Inches(2.5), Inches(0.04)
        )
        ul.fill.solid()
    ...
    def _add_content_slide(self, prs: Presentation, slide_data: Dict, page_num: int):
        ...
        tb = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(0), Inches(0), Inches(13.333), Inches(0.05)
        )
        tb.fill.solid()
        ...
        ttb = slide.shapes.add_textbox(
            Inches(0.5), Inches(0.25), Inches(12.3), Inches(0.75)
        )
        tf = ttb.text_frame
        tf.word_wrap = True
        p = tf.paragraphs[0]
        ...
        tul = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(0.5), Inches(0.95), Inches(5), Inches(0.04)
        )
        tul.fill.solid()
        ...
        # Accent dot
        dt = slide.shapes.add_shape(
            MSO_SHAPE.OVAL, Inches(5.6), Inches(0.93), Inches(0.08), Inches(0.08)
        )
        dt.fill.solid()
        ...
        pb = slide.shapes.add_textbox(
            Inches(12.3), Inches(7.15), Inches(0.8), Inches(0.3)
        )
        ...
        for block in blocks:
            ...
            current_top = self._render_block(
                slide, block, left, current_top, width, max_bottom
            )
    ...
    def _render_bullets(self, slide, block: Dict, left, top, width, max_bottom):
        ...
        for bullet in bullets[: LayoutConfig.MAX_BULLETS]:
            ...
            # Text
            bullet_text = truncate_text(
                clean_text(bullet), LayoutConfig.MAX_BULLET_CHARS
            )
            txBox = slide.shapes.add_textbox(
                left + Inches(0.28), top, width - Inches(0.3), Inches(item_height)
            )
            tf = txBox.text_frame
            tf.word_wrap = True
            p = tf.paragraphs[0]
    ...
    def _render_numbered(self, slide, block: Dict, left, top, width, max_bottom):
        ...
        for i, point in enumerate(points[: LayoutConfig.MAX_POINTS]):
            ...
            circle = slide.shapes.add_shape(
                MSO_SHAPE.OVAL, left, top + Inches(0.04), Inches(0.26), Inches(0.26)
            )
            circle.fill.solid()
            ...
            numBox = slide.shapes.add_textbox(
                left, top + Inches(0.05), Inches(0.26), Inches(0.24)
            )
            ...
            txBox = slide.shapes.add_textbox(
                left + Inches(0.36), top, width - Inches(0.4), Inches(item_height)
            )
            tf = txBox.text_frame
            tf.word_wrap = True
            p = tf.paragraphs[0]
    ...
    def _render_definition(self, slide, block: Dict, left, top, width, max_bottom):
        ...
        bg = slide.shapes.add_shape(
            MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, Inches(box_height)
        )
        bg.fill.solid()
        ...
        # Accent bar
        ab = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE,
            left + Inches(0.12),
            top + Inches(0.12),
            Inches(0.05),
            Inches(box_height - 0.24),
        )
        ab.fill.solid()
        ...
        if term:
            tBox = slide.shapes.add_textbox(
                left + Inches(0.3), top + Inches(0.1), width - Inches(0.5), Inches(0.32)
            )
    ...
    def _render_table(self, slide, block: Dict, left, top, width, max_bottom):
        ...
        table = slide.shapes.add_table(
            num_rows, num_cols, left, top, width, Inches(table_height)
        ).table
        ...
        for r, row in enumerate(rows[: num_rows - 1]):
            for c, val in enumerate(row[:num_cols]):
                ...
                cell.fill.fore_color.rgb = (
                    Colors.BG_GLASS if r % 2 == 0 else Colors.BG_SECONDARY
                )
                p = cell.text_frame.paragraphs[0]
    ...
    def _render_image_block(self, slide, block: Dict, left, top, width, max_bottom):
        ...
        if self.image_generator:
            try:
                print(f"[PPT] Attempting to fetch image for prompt: '{prompt[:80]}...'")
                image_data = self.image_generator(prompt)
                print(
                    f"[PPT] Successfully fetched image ({len(image_data) if image_data else 0} bytes)"
                )
            except Exception as e:
                error_message = str(e)
                print(f"[PPT] Failed to fetch image: {error_message}")
        else:
            error_message = "No image generator configured"
            print(f"[PPT] {error_message}")
        ...
        if image_data:
            try:
                ...
                if img_height > 0.5:
                    ...
                    print(f"[PPT] Image added to slide successfully")
                    return top + Inches(img_height + LayoutConfig.BLOCK_GAP)
            except Exception as e:
                error_message = f"Failed to add image to slide: {str(e)}"
                print(f"[PPT] {error_message}")

        # Render informative placeholder
        placeholder_height = 0.6
        if top + Inches(placeholder_height) > max_bottom:
            return top

        # Create a styled placeholder box
        placeholder = slide.shapes.add_shape(
            MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, Inches(placeholder_height)
        )
        placeholder.fill.solid()
        placeholder.fill.fore_color.rgb = Colors.BG_GLASS
        placeholder.line.color.rgb = Colors.GLASS_BORDER
        placeholder.line.width = Pt(1)

        # Add icon and text
        txBox = slide.shapes.add_textbox(
            left + Inches(0.3), top + Inches(0.15), width - Inches(0.6), Inches(0.3)
        )
        tf = txBox.text_frame
        tf.word_wrap = True
        p = tf.paragraphs[0]

        # Create informative placeholder text
        if error_message:
            placeholder_text = f"⚠ Image unavailable: {error_message[:40]}..."
        else:
            # Truncate prompt for display
            display_prompt = prompt[:60] + "..." if len(prompt) > 60 else prompt
            placeholder_text = f"🖼 Image: {display_prompt}"
        ...
        return top + Inches(placeholder_height + LayoutConfig.BLOCK_GAP)
    ...
    def _render_diagram_block(self, slide, block: Dict, left, top, width, max_bottom):
        ...
        if self.image_generator:
            try:
                # Enhance prompt for diagram search
                query = prompt if "diagram" in prompt.lower() else f"{prompt} diagram"
                print(
                    f"[PPT] Attempting to fetch diagram for prompt: '{query[:80]}...'"
                )
                image_data = self.image_generator(query)
                print(
                    f"[PPT] Successfully fetched diagram ({len(image_data) if image_data else 0} bytes)"
                )
            except Exception as e:
                error_message = str(e)
                print(f"[PPT] Failed to fetch diagram: {error_message}")
        else:
            error_message = "No image generator configured"
            print(f"[PPT] {error_message}")
        ...
        if image_data:
            try:
                ...
                if img_height > 0.5:
                    ...
                    print(f"[PPT] Diagram added to slide successfully")
                    return top + Inches(img_height + LayoutConfig.BLOCK_GAP)
            except Exception as e:
                error_message = f"Failed to add diagram to slide: {str(e)}"
                print(f"[PPT] {error_message}")
        ...
        # Create informative placeholder text
        if error_message:
            placeholder_text = f"⚠ Diagram unavailable: {error_message[:40]}..."
        else:
            # Truncate prompt for display
            display_prompt = prompt[:60] + "..." if len(prompt) > 60 else prompt
            placeholder_text = f"📊 Diagram: {display_prompt}"
